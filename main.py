...
# app = FastAPI(
#     title="Координатный конвертер",
#     description="API для автоматизированного преобразования координат между системами и генерации отчетов",
#     version="1.0.0"
# )
...
#     report += "## Общая формула преобразования\n\n"
#     report += r"""
# $$
# \begin{bmatrix}
# X' \\
...
#     report += f"## Подстановка параметров: {source_system} → {target_system}\n\n"
#     report += "| Параметр | Значение |\n|----------|----------|\n"
#     with open("parameters.json", "r", encoding="utf-8") as f:
#         params = json.load(f)[source_system]
#         for k, v in params.items():
#             report += f"| {k} | {v} |\n"
#     report += "\n"

#     report += "## Пример вычисления первой точки\n\n"
#     first_before = df_before.iloc[0]
#     first_after = df_after.iloc[0]
#     report += f"| Поле | До | После |\n|------|----|-------|\n"
#     report += f"| Name | {first_before['Name']} | — |\n"
#     report += f"| X    | {first_before['X']:.3f} | {first_after['X']:.3f} |\n"
#     report += f"| Y    | {first_before['Y']:.3f} | {first_after['Y']:.3f} |\n"
#     report += f"| Z    | {first_before['Z']:.3f} | {first_after['Z']:.3f} |\n\n"

#     report += "## Таблица координат до и после преобразования\n\n"
#     report += "| Name | До (X,Y,Z) | После (X’,Y’,Z’) |\n|------|-------------|------------------|\n"
#     for i in range(min(10, len(df_before))):
#         b = df_before.iloc[i]
#         a = df_after.iloc[i]
#         report += f"| {b['Name']} | ({b['X']:.3f}, {b['Y']:.3f}, {b['Z']:.3f}) | ({a['X']:.3f}, {a['Y']:.3f}, {a['Z']:.3f}) |\n"
#     report += "\n"

#     report += "## Статистика\n\n"
#     report += f"- Количество обработанных точек: {len(df_before)}\n"
#     report += "- Преобразование выполнено успешно.\n"
...
#             "/generate-report/": "Генерация Markdown-отчета"
#         }
#     }
...
#         markdown_report = generate_markdown_report(df, df_result, source_system, target_system)
#         output = BytesIO(markdown_report.encode())
#         output.seek(0)
